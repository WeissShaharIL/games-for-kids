...
export default function Shooter({ player, players, onBack }) {
  ...
  const { phase, figure, countdown, scores={}, bullets={}, connected=[], figures_done=0, total=20 } = state
  ...
  if (phase === 'waiting') return (
    <div style={{ minHeight:'100vh', background:'#0f172a', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', fontFamily:'sans-serif' }}>
      ...
      <h2 style={{ fontSize:28, fontWeight:900, color:'#fff', margin:'12px 0 8px' }}>Quick Shot!</h2>
      <p style={{ color:'#64748b', fontSize:16, margin:'0 0 20px' }}>
        {bothHere ? 'Starting soon...' : 'Waiting for opponent...'}
      </p>
      <div style={{ display:'flex', gap:10 }}>
        {connected.map((name, i) => {
          const pi = safe(players, name, i)
          return (
            <div key={name} style={{ display:'flex', alignItems:'center', gap:8, background:`${pi.color}22`, border:`2px solid ${pi.color}44`, borderRadius:20, padding:'8px 16px' }}>
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#0f172a', display:'flex', flexDirection:'column', alignItems:'center', fontFamily:'sans-serif', userSelect:'none', position:'relative', overflow:'hidden' }}>
          ...
          {connected.map((name, i) => {
            ...
            return (
              <div key={name} style={{ display:'flex', alignItems:'center', gap:6 }}>
                ...
                <span style={{ fontWeight:900, color:pi.color, fontSize:22 }}>{scores[name]??0}</span>
              </div>
            )
          })}
        </div>
      ...
      <div style={{ width:'100%', height:4, background:'rgba(255,255,255,0.06)' }}>
        <div style={{ height:'100%', width:`${(figures_done/total)*100}%`, background:p.color, transition:'width 0.5s ease', boxShadow:`0 0 10px ${p.color}` }} />
      </div>
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', position:'relative', width:'100%', maxWidth:420, padding:'0 24px' }}>
        ...
        <div
          ...
          style={{
            ...
            background: figure
              ? isVillain
                ? 'radial-gradient(circle at 40% 35%, #3d0000, #1a0000)'
                : 'radial-gradient(circle at 40% 35%, #003d00, #001a00)'
              : 'rgba(255,255,255,0.03)',
            border: `3px solid ${figure ? isVillain ? '#ef4444' : '#22c55e' : 'rgba(255,255,255,0.07)'}`,
            boxShadow: figure
              ? isVillain
                ? '0 0 50px #ef444455, 0 0 100px #ef444422, inset 0 0 40px #ef444411'
                : '0 0 50px #22c55e55, 0 0 100px #22c55e22, inset 0 0 40px #22c55e11'
              : 'none',
        ...
        >
          {figure ? (
            ...
            <div style={{ color:'rgba(255,255,255,0.12)', fontSize:16, fontWeight:700, letterSpacing:2 }}>
              STAND BY...
            </div>
          )}
        </div>
