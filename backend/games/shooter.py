...
class ShooterGame:
    ...
    def shoot(self, player: str) -> bool:
        if self.phase != "playing":
            return False
        if self.bullets.get(player, 0) <= 0:
            return False
        ...
        if self.figure is None:
            ...
            self.last_shot = {"player": player, "result": "hit", "points": pts, "emoji": self.figure["emoji"]}
            self.figure = None
        else:
            pts = self.figure["points"]
            self.scores[player] = max(0, self.scores.get(player, 0) + pts)
            self.last_shot = {"player": player, "result": "wrong", "points": pts, "emoji": self.figure["emoji"]}
            self.figure = None
        return True
