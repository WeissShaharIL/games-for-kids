...
TOTAL_FIGURES  = 20
BULLETS        = 8
FIGURE_TIME    = 2.0   # seconds figure stays visible
BETWEEN_TIME   = 1.2   # seconds between figures
COUNTDOWN      = 3

# villain = shoot, dog/cat = don't shoot
FIGURES = [
...
class ShooterGame:
    ...
    def _reset(self):
        self.phase        = "waiting"
        self.countdown    = None
        self.figure       = None       # current figure or None
        self.figure_idx   = 0          # how many figures shown so far
        self.bullets      = {"Ariel": BULLETS, "Ella": BULLETS}
        self.scores       = {"Ariel": 0, "Ella": 0}
        self.hits         = {"Ariel": 0, "Ella": 0}   # correct hits
        self.wastes       = {"Ariel": 0, "Ella": 0}   # wrong shots
        self.last_shot    = None       # {player, result, emoji}
        self.figures_done = 0

    def shoot(self, player: str) -> dict | None:
        """Player fires a bullet. Returns result dict or None if can't shoot."""
        if self.phase != "playing":
            return None
        if self.bullets[player] <= 0:
            return None
...
async def game_loop():
    ...
    for i in range(TOTAL_FIGURES):
        ...
        # Wait for FIGURE_TIME or until shot
        elapsed = 0.0
        tick    = 0.1
