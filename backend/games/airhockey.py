...
W = 400; H = 700; GOAL_W = 120; GOAL_Y = 20
MALLET_R = 28; PUCK_R = 18; MAX_SPEED = 14; WIN_SCORE = 5
TICK_RATE = 1/60; COUNTDOWN = 3

def clamp(val, lo, hi): return max(lo, min(hi, val))

class Vec:
    def __init__(self, x=0.0, y=0.0): self.x = float(x); self.y = float(y)
    def dist(self, other): return math.hypot(self.x-other.x, self.y-other.y)
    def serialize(self): return {"x": round(self.x,2), "y": round(self.y,2)}
